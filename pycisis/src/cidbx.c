...
long rec_maxmfrl=MAXMFRL;	/* 25/02/97 */
long rec_mstload=0L;		/* 28/03/97 */
long trm_invload=0L;		/* 28/03/97 */

long fmt_fsiz=MAXMFRL;               /* fmt_inter() - to set max field length */

#if MULTI
#if BEFOREISIS301
long dbxfloff=0L;		/* dbxflock()/dbxulock() - offset */
long dbxflsiz=MSBSIZ;		/* dbxflock()/dbxulock() - length */
#else
long dbxfloff=MSBSIZ;		/* dbxflock()/dbxulock() - offset = blk #2 */
long dbxflsiz=MSBSIZ;		/* dbxflock()/dbxulock() - length */
#endif
#if BEFORE990317
...
#if PC
#if MSC
	if (lseek(fd,dbxfloff,SEEK_SET) != dbxfloff) fatal("dbxflock/lseek");
...
#if PC
#if MSC
    if (lseek(fd,dbxfloff,SEEK_SET) != dbxfloff) fatal("dbxulock/lseek");
...
#endif /* CICPP */
{
	...
	if (lseek(DBXmsopn,0L,SEEK_SET) != 0) fatal("dbxwlock/lock/lseek/ewl");
...
#endif /* CICPP */
{
 ...
 for (idbx=0L; idbx < ndbxs; idbx++)
  printf("dbxflush - ndbxs=%ld  idbx=%ld=%s=%p \n",
...
#if GEN_CORELEFT
#if CICPP
unsigned long DBXSTRU :: xcoreleft(void)
#else /* CICPP */
...
#if UNIX && !GCC
    long bytes;
    bytes=ulimit(3,0L)-(long)sbrk(0);
    if (dbxtrace) printf("+++ coreleft=%ld\n",bytes);
...
#if fpc_TRACE
	if (dbxtrace) printf("fpc_buffer: coreleft=%ld\n",CORELEFT());
#endif
    }
...
#endif
    vp=vci+strlen(vci);
    sprintf(vp,"/%ld/%d/%d",MAXMFRL,LE1,LE2);
    vp+=strlen(vp);
