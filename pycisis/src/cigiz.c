 ...
 printf(".."); GETCHAR;
    ...
    /* process tag list */
		...
		if (gmap->grngs >= MAXGIZR) fatal((char *)p);
                if (sscanf((CONST char *)q,"%ld/%ld",&xtag1,&xtag2) == 2) {
		    ...
		    continue;
                ...
                if (sscanf((CONST char *)q,"%ld",&xtag1) == 1) {
		    while (isdigit(*q)) q++;
...
long irec;          /* indice de vrecp, para info/store */
...
#if TRACXG1
 printf("recxpnmo - irec: %ld RDBxpn01,02: %d,%d\n",irec,RDBxpn01,RDBxpn02);
#endif
...
#if CICPP
long recxpn00 (RECSTRU *recp,
               int xpn01,
               int xpn02)
#else /* CICPP */
long recxpn00(irec,xpn01,xpn02)                                       /*
-------------
                    abenda se vrecp nao inicializado;
                    abenda se registro irec nao alocado;
                    abenda se registro irec invalido;
...
#if TRACYG1
 printf("recxpn00 - irec: %ld xpn01,02: %d,%d  SIZEOFTAG=%d\n",
...
if (getchar() != '\n') break;
    ...
    */
    ...
    for (irng=0, wocc=1; wocc <= dmap->drngs; wocc++, irng++) {
        ...
        dmap->drng1[irng]=0;
        if (sscanf((CONST char *)datp,"%ld/%ld",&xtag1,&xtag2) == 2)
            if (xtag1 >= 1 && xtag2 <= USHRT_MAX)
                if (xtag1 <= xtag2) {
                    dmap->drng1[irng]=(UWORD)xtag1;
                    dmap->drng2[irng]=(UWORD)xtag2;
                }
        if (sscanf((CONST char *)datp,"%ld",&xtag1) == 1)
            if (xtag1 <= USHRT_MAX) {
                dmap->drng1[irng]=(UWORD)xtag1;
...
#if TRACED2
printf("\\valmfn=%ld",valmfn); GETCHAR;
#endif
                /* decode */
                if (valmfn > 0L) {
#if CICPP
		    ...
		    R2RECORD(decrecp,(char *)dmap->ddbnp,valmfn);
                    ...
                    if (R2RECrc == RCNORMAL) { /* break */
                        nmatchs++; match=1;
#if TRACED2
printf("\\match #%ld",nmatchs); GETCHAR;
#endif
                        if (dectrace)
