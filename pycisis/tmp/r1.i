...
int NXPAGES[] = {1,10,1,1,1,1,1,1,1,1};
# 85 "cirun.h"
...
int main(int argc, char *argv[])
{
 ...
 for (mfn=1L; ; mfn++) {
  recp=vrecp[(unsigned)record(0L,argv[1],mfn)];
  if (recp->recrc == (-1)) break;
  if (recp->recrc != 0) continue;
  printf("mfn %ld\n",mfn);
  for (diridx=0, dirloop=recp->recmf.m1.m1nvf; dirloop--; diridx++) {
   printf("%3u  ",recp->recmf.m1.m1dir[diridx].tag);
   p=recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[diridx].pos);
   for (chrloop=recp->recmf.m1.m1dir[diridx].len; chrloop--; p++)
    printf("%c",*p);
   printf("\n");
  }
 }
