...
------------
		    ...
		    retorna RCNORMAL ou aborta
								      */
long irec;          /* vrecp[] element defining RECdbxp/MFRmfn */
int option;
#endif /* CICPP */
{
    RECSTRU *recp;
...
#if RUUTRACE
 printf("recunlck - irec=%ld  dbn=%s/%ld  unlck=%d\n",irec,DBXname,mfn,option);
#endif
    ...
    if (force) {
	if (lseek(DBXmsopn,0L,SEEK_SET) != 0) fatal("recunlck/force/lseek");
...
------------
		    ...
		    retorna RCNORMAL
								      */
long crec;          /* indice de vrecp, para recwrite() */
long irec;          /* indice de vrecp, para recwrite() */
#endif /* CICPP */
{
...
printf("recupdat - crec=%ld  irec=%ld  dbn(crec)=%s  mfn=%ld  opw=%d,%d\n",
 crec,irec,VRDBname(crec),mfn,VRDBxropw(crec),VRDBmsopw(crec));
    ...
    recp=vrecp[crec];
#endif /* CICPP */
    printf("recupdat - RECrc=%d  Ctlmfr: %ld,%ld,%ld,%d,%d,%ld,%ld,%ld,%ld\n",
	RECrc,
    ...
    recp=vrecp[irec];
#endif /* CICPP */
    printf("recupdat - RECrc=%d  Leader: %ld,%d,%ld,%d,%d,%d,%d\n",
...
#if RUCTRACE
printf("recwrite - dbn/%d=%s  mfn=%ld  nxtmfn=%ld  nxtmfb=%ld  nxtmfp=%d\n",
...
#if RUCTRACE
printf("recwrite - comb=%ld  comp=%d  new=%d  upd=%d  pend=%d  rc=%d\n",
comb,comp,pendnew,pendupd,pend,rc);
	...
	comp=MS0nxtmfp-1;
#if RUCTRACE
printf("recwrite - comb=%ld  comp=%d [%ld,%ld+%d]\n",
 comb,comp,MS0nxtmfn,MS0nxtmfb,MS0nxtmfp);
...
#endif /* MULTI */
	xbyte=(long)((comb-1)<<MSBSHIFT)+comp;
	if (lseek(DBXmsopn,xbyte,SEEK_SET) != xbyte)
	...
	if (n != LEADER) {
#if RUCTRACE
printf("recwrite - fd=%d  lseek=%ld  n=%d\n",DBXmsopn,xbyte,n);
#endif
	    fatal("recwrite/read");
	}
	if (MSHmfn != mfn)
...
#if RUCTRACE
printf("recwrite - rec: %ld,%d,%ld,%d,%d,%d,%d\n",
	MFRmfn,MFRmfrl,MFRmfbwb,MFRmfbwp,MFRbase,MFRnvf,MFRstatus);
printf("recwrite - mst: %ld,%d,%ld,%d,%d,%d,%d\n",
...
#if RUCTRACE
printf("recwrite - mfbwb=%ld  mfbwp=%d  inplace=%d  flagnew=%d\n",
...
#if RUCTRACE
printf("recwrite - thiscomb=%ld  thiscomp=%d\n",thiscomb,thiscomp);
printf("recwrite - MS0nxtmfn=%ld  MS0nxtmfb=%ld  MS0nxtmfp=%d\n",
...
#if RUCTRACE
printf("recwmast - fd=%d  lseek=%ld  newblk=%d\n",
 DBXmsopn,xbyte,newblk);
#endif
...
long lastmfn;       /* ultimo mfn em .xrf */
#endif /* CICPP */
{
    DBXSTRU *dbxp;
    long mfn;
...
#if RUCTRACX
printf("recwxref - lastmfn=%ld             \n",lastmfn);
printf("recwxref - lastpos=%ld  thispos=%ld\n",lastpos,thispos);
...
printf("recwxref - thispos == lastpos\n");
	...
	xbyte=(long)( ((thispos-1)<<XRSHIFT)+XRPOSSIZ+(thisidx-j)*XRPTRSIZ );
	if (lseek(DBXxropn,xbyte,SEEK_SET) != xbyte)
...
#if RUFTRACE
printf("fldupdat - irec=%ld  nbytes=%ld  batchp='%s'\n",
 irec,RECnbytes,batchp);
#endif

#define PROCG  0 // mover pra cima
#if PROCG
#include "ciupg.c"
#endif /* PROCG */
...
#if RUFTRACE
printf("fldupdat - RECrc=%d  Leader: %ld,%d,%ld,%d,%d,%d,%d\n",
 RECrc,MFRmfn,MFRmfrl,MFRmfbwb,MFRmfbwp,MFRbase,MFRnvf,MFRstatus);
...
#else
    if ((long)(MFRmfrl+(nadd-ndel)*sizeof(DIRSTRU)-vlendel+vlenadd) >
        (long)RECnbytes) { /* AOT/RP - 26/04/2000 - para ALPHACPU (IPEN) */
#endif
    freex(srcareap,dirarea);
	return(batchp);
    }
