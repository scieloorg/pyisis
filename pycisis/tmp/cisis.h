...
#if GCCx
#ifndef PACKED
#define PACKED __attribute__ ((aligned(2),packed))
#endif
#endif
...
#ifndef CIFFI
#define CIFFI       00                   /* size-t mfrl/base/pos/len */
#endif
#ifndef CIWTF
#if !CICPP
#if !CIAPI
...
typedef struct msstru {
    ...
    long     msbufn;            /* gdb .mst blk order - simulated */
} MSSTRU;
...
#define PUNT        long        /* gdb paho 1988 */

#define INFO        long        /* gdb */
...
typedef struct invmap {
    ...
    long     cc_offset;         /* .cnt SEEK_SET offset */
#if CNLI
    long     cn_offset[2];      /* .n0x SEEK_SET offset */
    long     cl_offset[2];      /* .l0x SEEK_SET offset */
    long     ci_offset;         /* .ifp SEEK_SET offset */
#endif
} INVMAP;
...
typedef struct dbxstru {
...
#endif
    long       dbxmsmfn;          /* mstsetup() .mst nxtmfn */
    ...
    long       dbxitell;          /* ifupdat() tell parameter */
    long       dbxirang;          /* ifupdat() tell parameter */
    long       dbxirecs;          /* ifupdat() records updated */
    long       dbxipadd[2];       /* ifupdat() added lk1/lk2 posts */
    long       dbxipdel[2];       /* ifupdat() deleted lk1/lk2 posts */
#endif
} DBXSTRU;
#endif /* CICPP */
...
extern long ndbxs;              /* actual #entries              */
extern long maxndbx;            /* runtime value                */
#endif /* CICPP */
...
extern long dbxfloff;           /* dbxflock()/dbxulock() - offset */
extern long dbxflsiz;           /* dbxflock()/dbxulock() - length */
...
extern long rec_maxmfrl;   /* MAXMFRL/cipar maxmfrl entry */
extern long rec_mstload;      /* 28/03/97 */
extern long trm_invload;      /* 28/03/97 */
...
typedef struct m0stru {
    long     m0ctlmfn    PACKED;/* gdb ctlmfn */
    long     m0nxtmfn    PACKED;/* gdb nxtmfn */
    long     m0nxtmfb    PACKED;/* gdb nxtmfb */
    UWORD      m0nxtmfp    PACKED;/* gdb nxtmfp - offset */
    UWORD      m0mftype    PACKED;/* gdb mftype */
#if PCREADLINUX
    UWORD      fillerXm0mftype;
#endif /* PCREADLINUX */
    long     m0reccnt    PACKED;/* gdb reccnt */
    long     m0mfcxx1    PACKED;/* gdb mfcxx1 */
    long     m0mfcxx2    PACKED;/* gdb mfcxx2 - MULTI: Data entry lock */
    long     m0mfcxx3    PACKED;/* gdb mfcxx3 - MULTI: Exclusive write lock */
} M0STRU;
...
/* PACKED CIFFI */
/* 4 */      long     m1mfn       PACKED;/* gdb mfn */
/* 2 4 */    FFI      m1mfrl      PACKED;/* gdb mfrl - MULTI: record being updated */
#if PCREADLINUX
             UWORD      fillerXm1mfrl;
#endif /* PCREADLINUX */
/* 4 */      long     m1mfbwb     PACKED;/* gdb mfbwb */
...
#else /* CIFFI */
#if ISPACKED || CNV_PCFILES     /* PC || VAX || CNV_PCFILES */
...
typedef struct recstru {
    long     recnbytes;         /* nbytes allocated (RECHSIZE not included) */
...
extern long nrecs;              /* actual #entries              */
extern long maxnrec;            /* runtime value                */
#endif /* CICPP */

#if !CICPP
extern int rectrace;            /* rec RESTRACE runtime switch  */

extern long recxrefb;           /* recxref() comb */
...
#if IFUPDATE
extern long recreadb;           /* force recread() comb = mfbwb */
...
long recgizmo(CISISX *cisisxp, RECSTRU *recp, VGIZPSTRU *vgizmap);
#else /* CICPP */
...
#if ANSI
#if CICPP
long recxpnmo(RECSTRU *recp);
long recxpn00 (RECSTRU *recp, int xpn01, int xpn02);
#else /* CICPP */
long recxpnmo(long irec);
long recxpn00(long irec, int xpn01, int xpn02);
#endif /* CICPP */
#else
long recxpnmo();
long recxpn00();
...
long recdecex (CISISX * cisisxp, RECSTRU *recp);
#else /* CICPP */
...
#endif
    INFO     info1 PACKED;             /* gdb .l01 idx info1 */
    INFO     info2 PACKED;             /* gdb .l01 idx info2 */
#if LIND
    union {
        INFO    info3 PACKED;          /* tlc info3 - tot no of docs */
        INFO    info4 PACKED;          /* tlc info4 - posting type and etc */
    } info3info4;
#endif
} L1IDXE;
...
typedef struct l2idxe {
    ...
    UCHR     keyalign[2] PACKED; /* see ISIS.PAS for UNISYS */
#endif
    INFO     info1 PACKED;             /* gdb .l02 idx info1 */
...
typedef struct lxidxe {
    INFO     info1;             /* gdb .l0x idx info1 */
    INFO     info2;             /* gdb .l0x idx info2 */
#if LIND
    union {
        INFO info3;             /* tlc info3 - tot no of docs */
        INFO info4;             /* tlc info4 - posting type and etc */
    } info3info4;
#endif
} LXIDXE;
...
typedef struct lzstru {
    ...
    PUNT     psb PACKED;               /* tlc gdb .l0x ps backward */
#endif
    INFO     info1[TWORDF] PACKED;     /* tlc gdb .l0x idx info1 */
    INFO     info2[TWORDF] PACKED;     /* tlc gdb .l0x idx info2 */
#if LIND
    INFO     info3[TWORDF] PACKED;     /* tlc gdb .l0x idx info3 */
#endif
...
typedef struct trmstru {
    ...
    long     trmrseek[2];       /* last term/nxterm leafread() lseek addr */
    ...
    long     trmpleft;          /* postings left in segment */
    UCHR     *trmpcurr;         /* trmpchar not yet incremented */
    UCHR     *trmpchar;         /* ptr to next posting in trmifbuf */
#if LIND
    long     trmyaddr;          /* byte address */
    long     trmypsts;          /* total no of psts */
    int      trmycase;          /* postings type */
    int      trmysize;          /* postings length */
    long     trmydocs;          /* total no of docs */
    long     trmydisk;          /* total no of bytes */
    long     trmyread;          /* total no of bytes already read */
    long     trmyleft;          /* total no of bytes left on ifbuf */
#else
...
long term(long itrm, UCHR *dbnamp, UCHR *keyp);
...
#else
IFPSTRU *postread(UCHR *xbufp, INVMAP *invp, long addr, unsigned int toread,
                                                                int lxx);
#endif
int cntread(DBXSTRU *dbxp, int *ifpopen, char *ifpname, int *firstcnli);
void invsetup(UCHR *dbnamp, long loadn0x,long loadl0x,long loadpst);
...
extern long ntrms;              /* actual #entries              */
extern long maxntrm;            /* runtime value                */
#endif /* CICPP */
#endif /* CIAPI */
...
int recwmast(RECSTRU *crecp, RECSTRU *recp, long b, int p, int nblk, FFI wlen);
int recwxref(RECSTRU *recp, XRPTR pointer, long lastmfn);
...
#if CNV_PCBINUM
int ifpwrit(DBXSTRU *dbxp, char *buffer, long nbytes, INFO hdblk[], INFO hdoff[], int hdn);
#else /* CNV_PCBINUM */
...
long prtxref(CISISX *cisisxp, RECSTRU *recp, long mfn);
long prtdir(RECSTRU *recp, long mfn);
int recfgets(CISISX *cisisxp, RECSTRU *crec, RECSTRU *irec, char line[], FFI linsiz, FILE *fpconv);
int reccopy(CISISX *cisisxp, RECSTRU *upirec,RECSTRU *upcrec,RECSTRU *recp,long mfn);
...
long prtdir(RECSTRU *recp, long mfn);
int recfgets(long crec, long irec, char line[], FFI linsiz, FILE *fpconv);
...
#define ISOBSIZ         99999L          /* wrn: is long */    /* AOT, 06/06/2005 - length[5] */
...
typedef struct pdlstru {
   ...
   long        pdlleft;    /* cuanto queda del lvel */
   long        pdldocs;    /* cantidad de documentos almacenados */
   long        pdllmfn;    /* valor del último MFN de la BD */
   long        pdlsize;    /* tamaño actual de lvel */
   long        pdlisiz;    /* tamaño inicial de lvel, also growing step */
...
typedef struct b7hitbuf {
    long setno;                         /* query number */
    long bufsiz;                        /* buffer size */
    long ndocs;                         /* number of docs */
    int  hcase;                         /* BITSTRING or MFNSTRING (4 bytes) */
    int  htype;                         /* list of long or unsigned short mfn's */
    long align4;                        /* 4-byte alignment */
    UBYTE area[1];                      /* buffer - via ALLOC() */
} B7HITBUF;

typedef struct b7hitlist {
    long mfnlist[1];                    /* array of mfn */
} B7HITLIST;
...
/* CIB40 Interface */
#if ANSI
char *b4_exp(long irec, char *dbnamp, char *qryp,char *qrydbnp, int *errnop);
char *b4_run(long irec, char *dbnamp, long nbytes1, long nbytes2, long tell,
                unsigned char uctab[], long tlirec, int *errnop);
long b4_hit(long irec, long nhit);
int b4_msg(int tline, char *msg, long psts, long docs, int tmsg);
int b4_kbh();
...
#if !CICPP
#if ANSI
char *b5_exp(long irec, char *dbnamp, char *qryp,
                char *buffup, char *qrydbnp, long crec, int *errnop);
#if BIGREC
char *b5_run(long irec, char *dbnamp,
                char *buffup, long nbytes1, long tell,
                unsigned char uctab[], char *btch0p, long tlirec, long crec,
                int *errnop);
long b5_hit(long irec, long nhit, char *buffup);
#else /* BIGREC */
char *b6_run(long irec, char *dbnamp,
                char *buffup, int buffuplen, long nbytes1, long tell,
                unsigned char uctab[], char *btch0p, long tlirec,
      long crec, int *errnop);
long b6_hit(long irec, long nhit, long setno, B6HITBUF *bufferp);
int b6_hballoc(long bufsiz, B6HITBUF **bufferpp);
int b6_hbinit(long setno, B6HITBUF *bufferp);
int b6_hbfree(B6HITBUF **bufferpp);
#endif /* BIGREC */
int b5_msg(int tline, char *msg, long docs, int tmsg);
int b5_kbh(void);
char *b5_massp(char *tken, int tklen, int b5ee_pfx);
void b5_experr(int error, char *errp, int errl);
int b50read(DBXSTRU *dbxp,long mfn,int tag,int occ,char *areap,long *areasizp);
int b5_pfx(long irec, UWORD invxtag, char *dp, char *op, int b5re_pfx);
...
extern long b5setno;            /* query set number */
...
extern long b5_isn1;            /* query #0 lower limit */
extern long b5_isn2;            /* query #0 upper limit */
...
#if CIAPI_SOURCE
long recfmt( RECSTRU* recp, long lw, char *fmt, char *area, long asize);
long recfmtcmp(RECSTRU* recp, long lw, char *fmt, char *area, long asize,
...
#if !CICPP
extern int fmttrace;
extern long fmt_error; /* error - code */
extern long fmt_errof; /* error - fmt spec offset to error */
extern long fmt_fsiz;  /* fmt_inter() - to set max field length */
#endif /* CICPP */
...
int fst_link(char *pfxp, int plen, char *keyp, int klen,
         long mfn, UWORD tag, UWORD occ, UWORD cnt,
         char *link1, char *link2, long *lft1, long *lft2);
int fst_writ(int treecase, long mfn, UWORD tag, UWORD occ, UWORD cnt,
         char *kp, int kl, UCHR *uctabp);
int fst_open(char *gidbnp, char *filnamp, int treecase);
int fst_clos(int treecase);
void pstdecod(POSTSTRU *pst, long *mfn, UWORD *tag, UWORD *occ, UWORD *cnt);
void prtlink(char *sp, long mfn, UWORD tag,UWORD occ,UWORD cnt, UCHR *keyp, int len);
#else
long recfst();
long recfstcmp();
long fst_gener();
...
#endif /* CICPP */
#if !CICPP
extern long fst_error;  /* gener - fst spec error */
extern long fst_errl;   /* gener - fst spec line where error occured */
extern char *fst_errp;  /* gener - fst spec ptr where error occured */
extern char *fst_fmtap; /* inter - static format area */
extern long fst_fmtby;  /* inter - bytes for dynamic format area */
extern char fst_rdlm;   /* inter - new occ delimiter */
extern int fst_fd[];    /* inter - file descriptor for .ln1/.ln2 */
extern long fst_fx[];   /* inter - output records to .ln1/.ln2 */
...
extern long fst_wlupitrm; /* inter - word lookup I/F for TW processing */
#endif /* CICPP */
...
#if LIND
long svdiflind(char *dbnp,char *filekeys_1,char *filekeys_2,
                  long maxmfn,char *logp,
...
/* MYSAOT Interface */
#if MYSFUN
int cisis_mysfunc(char *gidbnp, int parmlen, char *parmin, char *parmout, long parmtell);
#endif /* MYSFUN */
