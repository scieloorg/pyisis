...
#endif /* CICPP */
{
	...
	for (count=dbxiloop; count--; ) {
	    ...
	    if (lseek(DBXmsopn,0L,SEEK_SET) != 0) fatal("dbxilock/L/lseek");
	...
	for (count=dbxiloop; count--; ) {
	    ...
	    if (lseek(DBXmsopn,0L,SEEK_SET) != 0) fatal("dbxilock/UM/lseek/2");
#if DBXMSTXL /* AOT/HB 21/08/2002 */
    if (DBXmstxl) {
		((M0STRU *)m0area)->m0mftype += DBXmstxl * 256;
    }
...
#define GETACUCTAB 1
    ...
    p=dbxcipar(NULL,"maxmfrl",'='); if (dbxcipok) sscanf(p,"%ld",&rec_maxmfrl); if (rec_maxmfrl < (long)MSNVSPLT || (RECHSIZE+rec_maxmfrl) > ALLOMAXV) rec_maxmfrl=MAXMFRL;
#if CICPP

#else /* CICPP */
    fmt_fsiz=rec_maxmfrl;
#endif /* CICPP */
    p=dbxcipar(NULL,"mstload",'='); if (dbxcipok) sscanf(p,"%ld",&rec_mstload);
    p=dbxcipar(NULL,"invload",'='); if (dbxcipok) sscanf(p,"%ld",&trm_invload);
...
#if DBXTRAC0
printf("dbxstorp - dbnamp='%s' DBXname='%s' p=%p ndbxs=%ld\n",
...
printf("dbxopenc  - [%s.par] '%s' \n",gidbnp,filnamp);
    ...
    if (fd > 0)
        if (append || xappend) lseek(fd,0L,SEEK_END);
	else lseek(fd,0L,SEEK_SET);	/* rewind */
...
*/
{
    ...
    /* Rod Senra 2007-11-06
        - tmpnam replaced by mkstemp

      To remove the warning = 
      cidbx.c:(.text+0x2a13): warning: 
...
//				sprintf(mess_str,"%s %ld",resettingmess,mfn);
				sprintf(mess_str,"MFN: %ld",mfn);
	...
	if (flagnew || flagmod) {
	    ...
	    xbyte=(long)( ((thispos-1)<<XRSHIFT)+XRPOSSIZ+thisidx*XRPTRSIZ );
	    if (lseek(DBXxropn,xbyte,SEEK_SET) != xbyte)
...
printf(" opn=%d opw=%d\n",invp->cnopn,invp->cnopw);
    ...
    /* read .cnt into DBIcn[] */
#if BEFORE950820
    if (lseek(fd,0L,SEEK_SET) != 0) fatal("cntread/seek"); /* see CNLI */
#endif
#if CNV_PCFILES
...
printf("+++cnt1:\n");
    cp=(CNSTRU *)p;
                ...
                printf("liv=%d posrx=%ld\n",
                        cp->liv,cp->posrx);
                printf("nmaxpos=%ld fmaxpos=%ld abnormal=%d\n",
...
*/
long loadn0x;       /* carga total de .n0x */                        /* v3.3
*/
long loadl0x;       /* carga total de .n0x */                        /* v3.3
*/
long loadpst;       /* carga total de .ifp */                        /* v3.3
*/
#endif /* CICPP */
{
...
#if TRSXRACE
if (trmtrace) {
printf("invsetup - nybasep[%d]=%p %6ld/%6ld=%6ld bytes/pages  fd=%d\n",
...
#if TRSXRACE
if (trmtrace) {
printf("invsetup - nybasep[%d]=%p %ld pages\n",
...
#if CNLI
		/* .... 951208 */
		lseek(fd,invp->cn_offset[treecase],SEEK_SET);
#endif
...
#if TRSTRACE
if (trmtrace && dbxtrace) {
printf("invsetup - nxp[%d][%d]=%p com %ld bytes\n",treecase,level,nxp,nbytes);
}
#endif
		}
	    }
...
#if TRSXRACE
if (trmtrace) {
printf("invsetup - lybasep[%d]=%p %6ld/%6ld=%6ld bytes/pages  fd=%d\n",
...
#if TRSXRACE
if (trmtrace) {
printf("invsetup - lybasep[%d]=%p %ld pages\n",
...
#if CNLI
		/* .... 951208 */
		lseek(fd,invp->cl_offset[treecase],SEEK_SET);
#endif
...
#if CNLI
	    /* .... 951208 */
	    loop=lseek(fd,invp->ci_offset,SEEK_SET);
#endif
#if !LIND
...
#endif
	    lseek(fd,-(long)sizeof(IFPCTRL),SEEK_CUR);
...
#else /* LIND */
#if BEFORE951208
	    nbytes=lseek(fd,0L,SEEK_CUR);
#else
	    nbytes=lseek(fd,0L,SEEK_END);
#if CNLI
	    nbytes-=loop;
#endif
	    lseek(fd,loop,SEEK_SET);
#endif
...
#if TRSXRACE
if (trmtrace) {
printf("invsetup - iybasep   =%p %6ld/%6ld=%6ld bytes/pages  fd=%d\n",
...
#if TRNTRACE
printf("noderead - invp=%p  treecase=%d  level=%d  punt=%ld\n",
    invp,treecase,level,(long)punt);
#if CNLI
printf("noderead - fd=%d  offset=%ld\n",fd,invp->cn_offset[treecase]);
#endif
#endif
    nxp= &invp->nx[treecase][level];
...
printf("noderead - nodesize=%d  fd=%d  total=%d  left=%d  basep=%p\n",
	...
	for (n=used, p=basep; n--; p+=nodesize) {
	    np=(N0STRU *)p;
#if TRNTRACE
printf("noderead - teste com np->pos=%ld\n",(long)np->pos);
#endif
	    if (np->pos == punt)
...
#endif
    xbytes=(punt-1L)*nodesize;
#if CNLI
    xbytes+=invp->cn_offset[treecase];
#endif
#if TRNTRACE
printf("noderead - xbytes=%ld nodesize=%d \n",xbytes,nodesize);
#endif
    if (lseek(fd,xbytes,SEEK_SET) != xbytes)
	fatal("noderead/lseek");
#if TRNTRACE
printf("noderead - read em %p (punt=%ld)\n",p,punt);
#endif
#if CNV_PCFILES
...
#endif /* CICPP */
{
    ...
    lbufp=fbufp;
#if TRLTRACE
printf("leafread - treecase=%d  punt=%ld  lbufp=%p  lxx=%d\n",
 treecase,(long)punt,lbufp,lxx);
#if SAMEL
...
#if SAMTRACE
printf("leafread - treecase=%d lbytes=%d+%d*%d=%ld\n",
...
printf("leafread - treecase=%d lbytes=%d+%d*%d=%ld\n",
 treecase,sizeof(LXL1STRU),invp->iflxn-1,sizeof(lxl1p->idx),lbytes);
	...
	lbufp=iflxp;                                    /* set */
#if SAMTRACE
printf("leafread - treecase=%d  punt=%ld  lxx=%d lxn=%d bytes=%ld/%p/%d\n",
 treecase,(long)punt,lxx,invp->iflxn,lbytes,lbufp,lbufsiz);
#endif
    }
#endif /* SAMEL */
    lp=(L0STRU *)lbufp;                                 /* use */
#if TRLTRACE
printf("leafread - treecase=%d  punt=%ld  lbufp=%p  lp=%p\n",
 treecase,(long)punt,lbufp,lp);
#if CNLI
printf("leafread - fd=%d  offset=%ld\n",fd,invp->cl_offset[treecase]);
#endif
#endif
    /* might ignore possible old leaf's ... */
    if (treecase != (lp->it-1) || punt != lp->pos) {
#if TRLTRACE
printf("leafread - invp=%p  treecase=%d  punt=%ld\n",
    invp,treecase,(long)punt);
#endif
	...
	if (p != NULL) {
	    p+=(lbufsiz*(punt-1L));
#if TRLTRACE
printf("leafread - lybasep[%d][%ld]=%p  p=%p  lp=%p\n",
...
#endif
#if TRLTRACE
printf("leafread - moved pos=%ld ock=%d it=%d ps=%ld\n",
 lp->pos,lp->ock,lp->it,(long)lp->ps);
#endif
	}
	else {                                                  /* v3.3 */
#if LEAFCNV_PCFILES
	    lbufsiz-=(treecase)?TWORDF*L2BUNI:TWORDF*L1BUNI;
	    xbytes=(punt-1L)*lbufsiz;
#if TRLTRACE
printf("leafread - xbytes=%ld lbufsiz=%d \n",xbytes,lbufsiz);
#endif
	    if (lseek(fd,xbytes,SEEK_SET) != xbytes)
...
#endif
if (trmtrace) {
 printf("leafread - treecase=%d punt=%ld pos=%ld ock=%d it=%d ps=%ld",
  treecase,punt,(long)lp->pos,lp->ock,lp->it,(long)lp->ps);
#if LIND
 printf("/%ld \n",(long)lp->psb);
#else
 printf(" \n");
...
if (treecase) {
 ...
 getchar();
	...
	if (*p != '\0') {
	    ...
	    printf("leafread/check/punt=%ld -> char=%c(%02x)",punt,*p,*p);
	    if (getchar() != '\n') fatal("leafread/getchar");
	}
    }
#if TRLTRACE
printf("leafread - ock=%d  it=%d  ps=%ld\n",lp->ock,lp->it,(long)lp->ps);
#endif
    return(lp);
}
...
#if TRYTRACE
printf("postread - invp=%p  xblk=%ld  xbufp=%p  xp=%p  fd=%d \n",
 invp,(long)xblk,xbufp,xp,fd);
#endif
#else
    xbufsiz=toread;
#if TRYTRACE
printf("postread - invp=%p  addr=%ld/%d  xbufp=%p  xp=%p  fd=%d \n",
 invp,addr,xbufsiz,xbufp,xp,fd);
#endif
#endif
...
#endif
#if TRYTRACE
printf("postread - sbyte=%ld \n",sbyte);
#endif
	    if (lseek(fd,sbyte,SEEK_SET) != sbyte)
