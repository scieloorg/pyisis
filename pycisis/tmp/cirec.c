...
#if IFUPDATE
long recreadb=0;                /* force recread() comb = mfbwb */
...
#if CICPP
long RECSTRU :: xrecalloc(long nbytes)
#else
...
-------------
		    ...
		    retorna irec
								      */
long irec;          /* indice de vrecp, para ALLOC */
long nbytes;        /* tamanho da area util em bytes */
#endif /* CICPP */
{
    RECSTRU *recp;
...
#if REATRACE
#if CICPP
 printf("recallok - recp=%p  RECtype=%d RECnbytes=%ld\n",
    recp,RECtype,RECnbytes);
#else
 printf("recallok - vrecp[%ld]=%p nrecs=%ld RECtype=%d RECnbytes=%ld\n",
...
#if CICPP
long RECSTRU :: xrecord(char *dbnamp,
			long  mfn)
#else /* CICPP */
...
-----------
		    ...
		    retorna irec
								      */
long irec;          /* indice de vrecp, para store */
char *dbnamp;       /* ptr nome base de dados a ler */
long mfn;           /* numero do registro a ler */
#endif /* CICPP */
{
    RECSTRU *recp;
    DBXSTRU *dbxp;
    ...
    p=dbxcipar(NULL,"maxmfrl",'='); if (dbxcipok) sscanf(p,"%ld",&rec_maxmfrl);
...
#if RECTRACE
printf("record - irec=%ld  recp=%p  dbxp=%p=%s  mfn=%ld\n",
...
#if RECTRACE
if (RECtype == TYPEMF0) {
    printf("record - RECrc=%d  Ctlmfr: %ld,%ld,%ld,%d,%d,%ld,%ld,%ld,%ld\n",
	RECrc,
	MF0ctlmfn,MF0nxtmfn,MF0nxtmfb,MF0nxtmfp,
	MF0mftype,MF0reccnt,MF0mfcxx1,MF0mfcxx2,MF0mfcxx3);
}
else {
    printf("record - RECrc=%d  Leader: %ld,%d,%ld,%d,%d,%d,%d\n",
...
#if RERTRACE
printf("recread - recp=%p  dbxp=%p  mfn=%ld\n",recp,dbxp,mfn);
#endif
...
#if RERTRACE
printf("recread - recp=%p  dbxp=%p  mfn=%ld  comb=%ld  comp=%d  rc=%d\n",
...
#endif
	xseek=(long)((comb-1)<<MSBSHIFT);
	if (msyyp == NULL) {                                    /* v3.3 */
#if RERTRACE
printf("recread - fd,lseek=%d,%ld\n",fd,xseek);
#endif
	    if (lseek(fd,xseek,SEEK_SET) != xseek)
		fatal("recread/lseek");
...
#if RERTRACE
printf("recread - msyyp=%p fd,lseek=%d,%ld\n",msyyp,fd,xseek);
#endif
...
printf("recread- RECtype=%d  n1=%d  n=%d  MFX=%p  inp=%p\n",
	...
	if (REClock) {
	    ...
	    if (lseek(fd,0L,SEEK_SET) != 0) fatal("recread/lock/lseek/ctl");
#if CNV_PCBINUM
...
printf("recread - moveok=%d  n=%d  n2=%d  p=%p \n",moveok,n,n2,p);
	...
	xseek=(long)((comb-0)<<MSBSHIFT);
#if RERTRACE
printf("recread - fd,lseek2=%d,%ld\n",fd,xseek);
#endif
	if (msyyp == NULL) {                                    /* v3.3 */
	    if (lseek(fd,xseek,SEEK_SET) != xseek) fatal("recread/lseek2");
	    nr=n;
#if VAX
	    for (left=nr; left > 0; ) {
...
#if RERTRACE
printf("recread - p=%p msyyp=%p fd,lseek2=%d,%ld\n",p,msyyp,fd,xseek);
#endif
	...
	if (rc != RCLOCK) {
	    ...
	    xseek=(long)((comb-1)<<MSBSHIFT); xseek+=comp;
	    if (lseek(fd,xseek,SEEK_SET) != xseek)
...
long mfn;           /* numero do registro a ler */
long *comb;         /* block number of MF block containing the rec */
...
#if REXTRACE
printf("recxref - DBXxribp->xrxrpos=%ld\n",DBXxribp->xrxrpos);
#endif
...
#if REXTRACE
printf("recxref - para mfn=%ld  xrpos=%ld  xrftiv=%ld\n",mfn,
DBXxribp->xrxrpos,xrftiv);
#endif
	}
	else {
	    xbyte=(newpos-1)<<XRSHIFT;
	    if (xryyp == NULL) {
		if (lseek(DBXxropn,xbyte,SEEK_SET) != xbyte)
		    fatal("recxref/lseek");
#if REXTRACE
printf("recxref - mfn=%ld  newpos=%ld  lseek=%ld  DBXmsmfn=%ld\n",
 mfn,newpos,xbyte,DBXmsmfn);
#endif
...
#if REXTRACE
printf("recxref - xryyp=%p mfn=%ld  newpos=%ld  lseek=%ld  DBXmsmfn=%ld\n",
 xryyp,mfn,newpos,xbyte,DBXmsmfn);
#endif
		xryyp+=(xbyte);
		n=XRBSIZ;
...
#if REXTRACE
printf("recxref - xrftiv=%ld,%08lx  comb=%ld  comp=%d  rc=%d\n",
...
long loadxrf;       /* carga total de .xrf */                        /* v3.3 */
long loadmst;       /* carga total de .mst */                        /* v3.3 */
#endif /* CICPP */
{
    DBXSTRU *dbxp;
    int fd;
...
#endif
	    nbytes=lseek(fd,0L,SEEK_END); lseek(fd,0L,SEEK_SET);
...
printf("mstsetup - DBXxryyp  =%p %6ld/%6ld=%6ld bytes/pages  fd=%d\n",
 DBXxryyp,nbytes,npages,nbytes/npages,fd);
}
#endif
	...
	DBXxribp->xrxrpos=0;
#if REYTRACE
printf("mstsetup - DBXxryyp  =%p  DBXxropn=%d  DBXxribp=%p [%ld]\n",
...
printf("mstsetup - DBXmsyyp  =%p %6ld/%6ld=%6ld bytes/pages  fd=%d\n",
 DBXmsyyp,nbytes,npages,nbytes/npages,fd);
	...
	if (strcmp(DBXname,"null") != 0) {
	    ...
	    if (lseek(DBXmsopn,SEEK_SET,0L)) fatal("mstsetup/seek/msibp");
...
#if REYTRACE
printf("mstsetup - DBXmsyyp  =%p  DBXmsopn=%d  DBXmsibp=%p [%ld]\n",
...
RECSTRU * RECSTRU :: xRecAlloc (long nbytes)
{
 xrecalloc(nbytes);
 return this;
}
