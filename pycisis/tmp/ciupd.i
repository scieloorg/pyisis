...

{
 ...
 fatal("recisis0/MONONETS expected");
    ...
    areap=(char *)malloc((unsigned)n);

    if (areap == (char *)((void *)0))
...
# 1617 "cisisx.c"
 ...
 fatal("recisis0/mst");
    ...
    strcpy(areap,dbnamp);
    strcat(areap,"/mstxl");
    strcpy(areap,dbxcipar(((void *)0),areap,'='));
    if (dbxcipok) n=atoi(areap); else n=cipmstxl;
    if (n) if (n < 0 || n > 4) fatal("recisis0/mstxl");
    if (n == 4) n=3;
    ...
    ms0p->m0nxtmfn=1;
    ms0p->m0nxtmfb=1;
    ms0p->m0nxtmfp=sizeof(M0STRU)+sizeof(M0STRU)+1;
    ms0p->m0mftype=0;
 ...
 recisis0_s = 0;
    ...
    ms0p->m0reccnt=ms0p->m0mfcxx1=ms0p->m0mfcxx2=ms0p->m0mfcxx3=0;




    memset(areap+sizeof(M0STRU),0x00,512 -sizeof(M0STRU));
...

{
    ...
    int flagnew,flagmod;
    int w,thisidx;
    long thispos,xbyte,count=0L,count2=0L;
# 1733 "cisisx.c"
    if (rectrace) printf("recreset - %s / %ld / %ld \n",dbnp,mfn1,mfn2);
 ...
 for (irec=maxnrec; --irec > 0; )
     if (!vrecp[irec]) {
  recallok(irec,32767L);
  recp=vrecp[irec];
  break;
     }
 if (irec == 0) fatal("recreset/maxnrec");
 record(irec,dbnp,mfn=mfn1);
 ...
 mfn=mfn2=recp->recmf.m1.m1mfn;
    }
    if (rectrace) printf("recreset : %s / %ld / %ld \n",dbnp,mfn1,mfn2);

    dbxp=recp->recdbxp;
    dbxopenw(dbxp->dbxname,dbxp->dbxname,xx1extp,&dbxp->dbxxropn,&dbxp->dbxxropw,"recreset/xropn/w");
    dbxopenw(dbxp->dbxname,dbxp->dbxname,mx1extp,&dbxp->dbxmsopn,&dbxp->dbxmsopw,"recreset/msopn/w");


    if (dbxp->dbxnetws != 0) if (!dbxp->dbxewlxx) fatal("recreset/EWL is off");


    if (mfn1) dbxp->dbxirecs=0;

    for (; mfn <= mfn2; mfn++) {

 if (mfn1) {
     if (rectrace) printf("recreset - reading %s / %ld \n",recp->recdbxp->dbxname,mfn);
     recreadl=1;
 ...
 if (rectrace) printf("recreset - bw=%ld/%d f=%d/%d\n",recp->recmf.m1.m1mfbwb,recp->recmf.m1.m1mfbwp,flagnew,flagmod);

 if (recp->recrc == (-1)) break;
...
# 1820 "cisisx.c"
 ...
 if (recp->recrc == 2) continue;

 w=0;

 if (recp->recmf.m1.m1mfbwb || recp->recmf.m1.m1mfbwp) {
     recp->recmf.m1.m1mfbwb=0;
     recp->recmf.m1.m1mfbwp=0;


            if (recp->recgdbw) recp->recmf.m1.m1mfrl=(unsigned short)0-recp->recmf.m1.m1mfrl;

     if (rectrace) printf("recreset - writing mst %s \n",recp->recdbxp->dbxname);

     recwmast(((void *)0),recp,recxrefb,recxrefp,0,1);
 ...
 if (flagnew || flagmod) {
     ...
     if (rectrace) printf("recreset - writing xrf %s  DBXmstxl=%d XRXDIVIDE=%d recxrefb=%ld recxrefp=%d\n",recp->recdbxp->dbxname,dbxp->dbxmstxl,2048,recxrefb,recxrefp);
        xrftiv = recxrefb * (2048>>dbxp->dbxmstxl) + (recxrefp>>dbxp->dbxmstxl);



     if (recp->recmf.m1.m1status == 1) xrftiv= -xrftiv;
     thispos=(recp->recmf.m1.m1mfn+126)/127;
     thisidx=(recp->recmf.m1.m1mfn-1)%127;
     xbyte=(long)( ((thispos-1)<<9)+sizeof(long)+thisidx*sizeof(long) );
  ...
  fatal("recreset/recwxref/seek");
     ...
     if (write(dbxp->dbxxropn,(char *)&xrftiv,sizeof(long)) != sizeof(long))

  fatal("recreset/recwxref/write");
     if (labs(dbxp->dbxxribp->xrxrpos) == thispos)
  dbxp->dbxxribp->xrmfptr[thisidx]=xrftiv;
     w++;
 }
 if (w) {count2++; if (mfn1) dbxp->dbxirecs++; }
    }
    if (parmtell)
 if (count % parmtell)
# 1884 "cisisx.c"
 ...
 if (rectrace) printf("+++ recreset - free %ld \n",irec);
 free(vrecp[irec]); vrecp[irec]=((void *)0); nrecs--;
...
# 123 "ciupd.c"
    ...
    dbxp=recp->recdbxp;
    mfn=recp->recmf.m1.m1mfn;


    oldgdbl=recp->recgdbl;
# 143 "ciupd.c"
 ...
 if (!vrecp[wrec]) break;
    ...
    vrecp[wrec]->recdbxp=dbxp;
    recp=vrecp[wrec];



    if (option & 8) { force=1; option-=8; }

    if (option & 3) option = (mfn) ? 1 : 2;
 ...
 fatal("recunlck/option");
    ...
    if (dbxp->dbxnetws == 0) {
 if (multrace)
     printf("<u> %s/%ld [%s MONONETS]\n",
       dbxp->dbxname,mfn,(option&3)?((option==1)?"RL":"DEL"):"EWL");
 return(0);
    }

    if (!dbxp->dbxmsopn) fatal("recunlck/msopn");
    dbxopenw(dbxp->dbxname,dbxp->dbxname,mx1extp,&dbxp->dbxmsopn,&dbxp->dbxmsopw,"recunlck/msopn/w");
    fd=dbxp->dbxmsopn;
...
# 191 "ciupd.c"
 ...
 if (dbxewlrc) return((-2));
 fatal("recunlck/file lock");
    }

    if (force) {
 if (lseek(dbxp->dbxmsopn,0L,0) != 0) fatal("recunlck/force/lseek");
 n=read(dbxp->dbxmsopn,recp->recmf.mx,sizeof(M0STRU));
 ...
 if (n != sizeof(M0STRU)) {
    ...
    memcpy(dbxp->dbxmsibp,recp->recmf.mx,sizeof(M0STRU));



    if (option & 3 && (!recp->recmf.m0.m0mfcxx2 || !dbxp->dbxdelxx) && !force) {
 if (multrace)
     printf("<u> %s/mfcxx2=%ld delxx=%d\n",dbxp->dbxname,recp->recmf.m0.m0mfcxx2,dbxp->dbxdelxx);
 ...
 if (option == 4) {
     ...
     if (recp->recmf.m0.m0mfcxx3 || dbxp->dbxewlxx) {
  if (recp->recmf.m0.m0mfcxx3) recp->recmf.m0.m0mfcxx3--;
  if (dbxp->dbxewlxx) dbxp->dbxewlxx--;
  ...
  uok=1;
     ...
     if (recp->recmf.m0.m0mfcxx2 || dbxp->dbxdelxx) {
  if (recp->recmf.m0.m0mfcxx2) recp->recmf.m0.m0mfcxx2--;
  if (dbxp->dbxdelxx) dbxp->dbxdelxx--;
 ...
 if (uok) {
  ...
  fatal("recunlck/lseek/ctl");
    ...
    if (dbxp->dbxmstxl) {
  ((M0STRU *)dbxp->dbxmsibp)->m0mftype += dbxp->dbxmstxl * 256;
    }
     ...
     if (write(fd,dbxp->dbxmsibp,sizeof(M0STRU)) != sizeof(M0STRU))

  fatal("recunlck/write/ctl");

    if (dbxp->dbxmstxl) {
  ((M0STRU *)dbxp->dbxmsibp)->m0mftype &= 0x00FF;
    }
 ...
 if (rc == (-1) || rc == 2) fatal("recunlck/EOFPDEL");





 if (oldgdbl || (force && recp->recgdbw)) {
     if (multrace)
       printf("<u> %s/%ld mfrl=%04x=+%d\n",dbxp->dbxname,mfn,recp->recgdbl,recp->recmf.m1.m1mfrl);




     recwmast(((void *)0),recp,comb,comp,0,1);
...
# 319 "ciupd.c"
 ...
 free(vrecp[wrec]); vrecp[wrec]=((void *)0); nrecs--;
    }
    recp=vrecp[irec];
...
# 372 "ciupd.c"
 ...
 fatal("recupdat/RECINIT");
    ...
    if (recp->recmf.m0.m0nxtmfn < 1) fatal("recupdat/crec/nxtmfn");

    if (recp->recmf.m0.m0nxtmfb < 1) fatal("recupdat/crec/nxtmfb");

    if (recp->recmf.m0.m0mftype != 0 &&
 recp->recmf.m0.m0mftype != 1) fatal("recupdat/crec/mftype");

    if (recp->recmf.m0.m0nxtmfn == 1)
        if (recp->recmf.m0.m0mftype != 1)
            if (recp->recmf.m0.m0nxtmfp <= sizeof(M0STRU)) fatal("recupdat/crec/nxtmfp/1");

    if (recp->recmf.m0.m0nxtmfp > 512) fatal("recupdat/crec/nxtmfp/MSBSIZ");
 ...
 fatal("recupdat/TYPEMF0");
    ...
    if ((mfn=recp->recmf.m1.m1mfn) < 1 || mfn > 0x00FFFFFFL)

      if (!(recp->recwlock & 4 && recp->recmf.m1.m1mfn == 0))

 fatal("recupdat/mfn");
# 420 "ciupd.c"
    recp->recdbxp=crecp->recdbxp;
...
# 524 "ciupd.c"
    ...
    if (!dbxp->dbxxropn || !dbxp->dbxmsopn) mstsetup(dbxp->dbxname,0L,0L);


    dbxopenw(dbxp->dbxname,dbxp->dbxname,xx1extp,&dbxp->dbxxropn,&dbxp->dbxxropw,"recwrite/xropn/w");
    dbxopenw(dbxp->dbxname,dbxp->dbxname,mx1extp,&dbxp->dbxmsopn,&dbxp->dbxmsopw,"recwrite/msopn/w");

    if (!dbxp->dbxxropw) fatal("recwrite/xropw");
    if (!dbxp->dbxmsopw) fatal("recwrite/msopw");

    if (!dbxp->dbxxribp) fatal("recwrite/xribp");
    if (!dbxp->dbxmsibp) fatal("recwrite/msibp");
    ...
    if (ms0p->m0mftype != 0 && ms0p->m0mftype != 1)
 fatal("recwrite/mftype");
    ...
    isnewrec=recp->recwlock & 4;

    if (dbxp->dbxnetws != 0) {
 if ((recp->recwlock & 1 && recp->recwlock & 2) ||
    !(recp->recwlock & 1 || recp->recwlock & 2))
     fatal("recwrite/WLOCK/WUNLOCK");


 if (!dbxp->dbxdelxx) fatal("recwrite/lock/data entry lock is off");

 if (!isnewrec)
     if (!recp->recgdbl) fatal("recwrite/lock/record lock needed");
...
# 585 "ciupd.c"
 ...
 if (!ms0p->m0mfcxx2) fatal("recwrite/lock/data entry lock/mfcxx2");
 if (!dbxp->dbxdelxx) fatal("recwrite/lock/data entry lock needed");


 if (isnewrec) {
     if (!mfn) {
         mfn=recp->recmf.m1.m1mfn=ms0p->m0nxtmfn;
         if (multrace) printf("<w> %s -> mfn=%ld\n",dbxp->dbxname,recp->recmf.m1.m1mfn);
     }
 }
    }


    lastmfn=ms0p->m0nxtmfn-1;

    if (recp->recdbxp != dbxp)
 fatal("recwrite/dbxp");

    rc=recxref(recp,mfn,&comb,&comp);

    if (comb > ms0p->m0nxtmfb)
 ...
 if (dbxp->dbxnetws != 0) {
     if (multrace) printf("<w> %s/%ld rc=%d\n",dbxp->dbxname,recp->recmf.m1.m1mfn,rc);
 ...
 memset((char *)mshp,0x00,20);

 mshp->m1status=1;
 comb=ms0p->m0nxtmfb;
 comp=ms0p->m0nxtmfp-1;
 ...
 if (dbxp->dbxnetws != 0)
     if (isnewrec)
  if (recp->recwlock & 8) ; else fatal("recwrite/lock/NORMALDEL");

 xbyte=(long)((comb-1)<<9)+comp;
 if (lseek(dbxp->dbxmsopn,xbyte,0) != xbyte)
     fatal("recwrite/lseek/leader");
 n=read(dbxp->dbxmsopn,(char *)mshp,20);
# 691 "ciupd.c"
 if (n != 20) {
 ...
 if (dbxp->dbxnetws != 0) {
     if (multrace)
       printf("<w> %s/%ld mfrl=%04x=%d\n",dbxp->dbxname,mfn,mshp->m1mfrl,mshp->m1mfrl);
     if (!isnewrec)
       if (mshp->m1mfrl <= (unsigned short)32767) fatal("recwrite/lock/record lock");
     if (mshp->m1mfrl > (unsigned short)32767) {
       mshp->m1mfrl=(unsigned short)0-mshp->m1mfrl;
       if (multrace)
        printf("<w> %s/%ld mfrl=%04x=+%d\n",dbxp->dbxname,mfn,0x0000,mshp->m1mfrl);
     }
 }
...
# 724 "ciupd.c"
    if (rc == (-1) || rc == 2)
 if (recp->recmf.m1.m1status == 1)
     fatal("recwrite/deleted");

    if (rc == (-1) || rc == 2 || pendnew)
 flagnew=1;
    else
 flagnew=0;
 ...
 recp->recmf.m1.m1mfbwb=mshp->m1mfbwb;
 recp->recmf.m1.m1mfbwp=mshp->m1mfbwp;
 inplace=1;
    }

    else {

 if (mshp->m1mfbwb != 0 && mshp->m1mfbwp != 0)
 ...
 if (rc != 0) {
     ...
     recp->recmf.m1.m1mfbwb=comb;
     recp->recmf.m1.m1mfbwp=comp;
     inplace=0;
 }
    }

    if (ms0p->m0mftype != 1) {
 if (recp->recmf.m1.m1mfrl & 0x01)
     recp->recmf.mx[recp->recmf.m1.m1mfrl++]=' ';

 if (dbxp->dbxmstxl == 2) {
     while (recp->recmf.m1.m1mfrl & 0x03)
         recp->recmf.mx[recp->recmf.m1.m1mfrl++]=' ';
 }
 if (dbxp->dbxmstxl == 3) {
     while (recp->recmf.m1.m1mfrl & 0x07)
         recp->recmf.mx[recp->recmf.m1.m1mfrl++]=' ';
 }
    ...
    if (dbxp->dbxnetws != 0) {
 if (recp->recmf.m1.m1mfrl > (unsigned short)32767) fatal("recwrite/lock/mfrl");
    }
# 787 "ciupd.c"
    if (recp->recmf.m1.m1mfrl <= mshp->m1mfrl && inplace == 1) {
 thiscomb=comb;
 thiscomp=comp;
 newblk=0;
    }
    else {
 thiscomb=ms0p->m0nxtmfb;
 thiscomp=ms0p->m0nxtmfp-1;

 if (thiscomp & 0x01 ||
    (dbxp->dbxmstxl == 2 && thiscomp & 0x03) ||
    (dbxp->dbxmstxl == 3 && thiscomp & 0x07))
 ...
 ms0p->m0nxtmfb=thiscomb;
 nxtcomp= (ms0p->m0mftype == 1) ? 512 : thiscomp+recp->recmf.m1.m1mfrl;
 for (;;)
     if (nxtcomp < 512)
  break;
     else {
 ...
 if (ms0p->m0mftype == 1)
    ...
    if (mfn >= ms0p->m0nxtmfn) {
 ms0p->m0nxtmfn=mfn+1;
 dbxp->dbxmsmfn=ms0p->m0nxtmfn;
    }
# 851 "ciupd.c"
    if (dbxp->dbxnetws != 0) {

 recp->recmf.m1.m1mfrl=(unsigned short)0-recp->recmf.m1.m1mfrl;
 recp->recgdbl=recp->recmf.m1.m1mfrl; recp->recgdbw=1;
 ...
 if (recp->recmf.m1.m1mfrl > (unsigned short)32767) {
     ...
     if (!recp->recgdbl || !recp->recgdbw) fatal("recupdat/check/gdbl");
     recp->recmf.m1.m1mfrl=(unsigned short)0-recp->recmf.m1.m1mfrl;
     if (multrace)
       printf("<w> %s/%ld mfrl=%04x=+%d\n",dbxp->dbxname,mfn,recp->recmf.m1.m1mfrl,recp->recmf.m1.m1mfrl);
 }
    ...
    xrftiv = comb * (2048>>dbxp->dbxmstxl) + (comp>>dbxp->dbxmstxl);
...

{
    ...
    if (crecp) dbxp=crecp->recdbxp; else dbxp=recp->recdbxp;
 ...
 if (lseek(dbxp->dbxmsopn,xbyte,0) != xbyte)
    ...
    if (wlen) {
 if (wlen == 1) mfrl=20;
 else if (wlen == 2) mfrl=recp->recmf.m1.m1base;
 else mfrl=wlen;
    }
    else {
 mfrl=recp->recmf.m1.m1mfrl;

 if (dbxp->dbxnetws != 0)
     if (recp->recmf.m1.m1mfrl > (unsigned short)32767)
  mfrl=(unsigned short)0-recp->recmf.m1.m1mfrl;
    ...
    if (recp) {
# 1018 "ciupd.c"
        if ((n=write(dbxp->dbxmsopn,recp->recmf.mx,mfrl)) != mfrl) {
    ...
    if (newblk) {
 k=512 -((xbyte+mfrl)&0x000001FF);




 memset(dbxp->dbxmsibp->msbuff,0x00,(size_t)k);

 if ((n=write(dbxp->dbxmsopn,dbxp->dbxmsibp->msbuff,k)) != k) {
    ...
    dbxp->dbxmsibp->msbufn=0;

    if (!crecp) return(0);

    lseek(dbxp->dbxmsopn,0L,0);

    recp=crecp;


    if (dbxp->dbxmstxl) {
        keepmftype=recp->recmf.m0.m0mftype;
        recp->recmf.m0.m0mftype += dbxp->dbxmstxl * 256;
    }
    ...
    n=write(dbxp->dbxmsopn,recp->recmf.mx,sizeof(M0STRU));
 ...
 fatal("recwmast/write/ctl");
    ...
    if (dbxp->dbxmstxl) {
        recp->recmf.m0.m0mftype=keepmftype;
    }
...

{
    ...
    lastpos=(lastmfn+126)/127;
    if (lastpos < 1)
 lastpos=1;
    flag127=0;
    if (lastmfn)
 if (mfn > lastmfn && (lastmfn % 127) == 0)
     { lastpos++; flag127=1; }

    thispos= (mfn+126)/127;

    lastidx=(lastmfn-1)%127;
    if (flag127)
 lastidx = -1;
    ...
    if (!lastmfn) recxref(recp,1L,&wcomb,&wcomp);

    if (thispos < lastpos) {
# 1164 "ciupd.c"
 if (dbxp->dbxxribp->xrxrpos == thispos) dbxp->dbxxribp->xrmfptr[thisidx]=pointer;
 ...
 if ((n=write(dbxp->dbxxropn,p,sizeof(long))) != sizeof(long)) {
 ...
 if (lastmfn)
     if (recxref(recp,lastmfn,&wcomb,&wcomp) == (-1))
  fatal("recwxref/recxref/lastmfn");

 x = (flag127) ? (lastpos-1) : (-lastpos);
 if (dbxp->dbxxribp->xrxrpos != x)
     fatal("recwxref/xrpos/lastpos");

 dbxp->dbxxribp->xrxrpos=lastpos;


    xrftiv = (-1) * (2048>>dbxp->dbxmstxl) + ((0)>>dbxp->dbxmstxl);




 for (n=lastidx+1; n<127; n++)
     dbxp->dbxxribp->xrmfptr[n] = xrftiv;

 xbyte=(long)((lastpos-1)<<9);
 if (lseek(dbxp->dbxxropn,xbyte,0) != xbyte)
 ...
 if ((n=write(dbxp->dbxxropn,(char *)dbxp->dbxxribp,512)) != 512) {
 ...
 dbxp->dbxxribp->xrxrpos= -thispos;

 for (n=0; n<thisidx; n++)
     dbxp->dbxxribp->xrmfptr[n] = xrftiv;

 dbxp->dbxxribp->xrmfptr[thisidx] = pointer;

 xrftiv= (0)*2048 + (0);

 for (n=thisidx+1; n<127; n++)
     dbxp->dbxxribp->xrmfptr[n] = xrftiv;
 ...
 xbyte=(long)( ((thispos-1)<<9)+sizeof(long)+(thisidx-j)*sizeof(long) );
 if (lseek(dbxp->dbxxropn,xbyte,0) != xbyte)
     fatal("recwxref/lseek/mfptrs");
 p=(char *)&dbxp->dbxxribp->xrmfptr[thisidx-j];
# 1323 "ciupd.c"
 if ((n=write(dbxp->dbxxropn,p,k)) != k) {
    ...
    if (mfn > lastmfn && (mfn % 127) == 0) {





 if (recxref(recp,mfn,&wcomb,&wcomp) == (-1))
     fatal("recwxref/recxref/mfn127");

 if (dbxp->dbxxribp->xrxrpos != -thispos) {
     printf("*** %ld/%ld ",dbxp->dbxxribp->xrxrpos,thispos);
     fatal("recwxref/xrpos/thispos");
 }

 dbxp->dbxxribp->xrxrpos=thispos;

 xbyte=(long)((thispos-1)<<9);
 if (lseek(dbxp->dbxxropn,xbyte,0) != xbyte)
 ...
 dbxp->dbxxribp->xrxrpos= -(thispos+1);
...

{
    ...
    extern int bugadddel;

    RECSTRU *recp;

    DIRSTRU *mfdirp,*dirarea=((void *)0),*dirp,*dp,*dxp;
    char *srcareap=((void *)0);
    unsigned short realmfrl;

    int maxdirs,dirsleft;
    unsigned short ndel,nadd;
    unsigned short vlendel,vlenadd;
    unsigned short lastpos,nxtpos;
    char *mfp,*nxtp,*newp;

    char *p,c,cc,*errp,*sp,*sxp;
    int tag;
    unsigned short n,k,loop,reploop;
    ...
    unsigned short smumaxlen=(unsigned short)32767;
    unsigned short fldlenmin=0;
    unsigned short fldleft,fldqleft,striplen,erased;
    long l1,l2;
    int nn1,nn2;
...
# 1640 "ciupd.c"
    nbytes = (recp->recnbytes > rec_maxmfrl) ? recp->recnbytes : rec_maxmfrl;





    dirarea=(DIRSTRU *)malloc((unsigned)nbytes);

    if (dirarea == (DIRSTRU *)((void *)0))
 fatal("fldupdat/ALLOC/dir");
    ...
    srcareap=(char *)malloc((unsigned)maxdirs);

    if (srcareap == (char *)((void *)0)) {
        { if (srcareap) { free(srcareap); srcareap=((void *)0); } if (dirarea) { free(dirarea); dirarea=((void *)0); } };
     fatal("fldupdat/ALLOC/src");
    }

    memset(srcareap,'?',maxdirs);


    dirsleft=recp->recmf.m1.m1nvf;

    realmfrl=recp->recmf.m1.m1base;

    sp=srcareap; dirp=dirarea;
    for (mfdirp=recp->recmf.m1.m1dir, n=dirsleft; n--; mfdirp++, dirp++, sp++) {
 dirp->tag = mfdirp->tag;
 dirp->pos = mfdirp->pos;
 dirp->len = mfdirp->len;
 *sp = 'r';
...
# 1687 "ciupd.c"
 ...
 case 'S':
  for (tag=0; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); p++) tag=tag*10+(*p-'0');
     if (*p) {
            { if (srcareap) { free(srcareap); srcareap=((void *)0); } if (dirarea) { free(dirarea); dirarea=((void *)0); } };
      return(errp);
     }
        if (tag) {
            int len,cmp;
      for (i=0; i+1 < recp->recmf.m1.m1nvf; i++) {
          if (recp->recmf.m1.m1dir[i].tag != tag) continue;
          for (j=i+1; j < recp->recmf.m1.m1nvf; j++) {
              if (recp->recmf.m1.m1dir[j].tag != tag) continue;
                    len=recp->recmf.m1.m1dir[i].len; if (len>recp->recmf.m1.m1dir[j].len) len=recp->recmf.m1.m1dir[j].len;
                    cmp=memcmp(recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[i].pos),recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[j].pos),len);
              if (cmp > 0 || cmp==0 && recp->recmf.m1.m1dir[i].len > recp->recmf.m1.m1dir[j].len) {
               memcpy(tempdir,&recp->recmf.m1.m1dir[i],sizeof(DIRSTRU));
               memcpy(&recp->recmf.m1.m1dir[i],&recp->recmf.m1.m1dir[j],sizeof(DIRSTRU));
               memcpy(&recp->recmf.m1.m1dir[j],tempdir,sizeof(DIRSTRU));
              }
                }
         }
         break;
     ...
     for (anychange=1; anychange; ) {
      ...
      for (i=0; i+1 < recp->recmf.m1.m1nvf; i++) {
          j=i+1;
          if (recp->recmf.m1.m1dir[i].tag > recp->recmf.m1.m1dir[j].tag) {
           memcpy(tempdir,&recp->recmf.m1.m1dir[i],sizeof(DIRSTRU));
           memcpy(&recp->recmf.m1.m1dir[i],&recp->recmf.m1.m1dir[j],sizeof(DIRSTRU));
           memcpy(&recp->recmf.m1.m1dir[j],tempdir,sizeof(DIRSTRU));
           anychange=1; sort=1;
 ...
 case '=':
     for (mfn=0; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); p++)
  mfn=mfn*10+(*p-'0');
     if (mfn == 0) {
        { if (srcareap) { free(srcareap); srcareap=((void *)0); } if (dirarea) { free(dirarea); dirarea=((void *)0); } };
  return(errp);
 ...
 case 'D':
  ...
  break;
     ...
     if (*p == ':') {
  recp->recmf.m1.m1status=(recp->recmf.m1.m1status==1)?0:1;
  p++;
  break;
     }
     if (*p == '*') {
  ...
  for (tag=0; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); p++)
      tag=tag*10+(*p-'0');
  if (tag == 0) {
            { if (srcareap) { free(srcareap); srcareap=((void *)0); } if (dirarea) { free(dirarea); dirarea=((void *)0); } };
      return(errp);
  ...
  while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace))
      p++;
  for (tagocc=0; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); p++)
 ...
 case '<':
  ...
  p++;
     ...
     if (c == 'H') {
  for (nn=n=0; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); p++, nn++)
      n=n*10+(*p-'0');



  if (!nn || (n && !((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace))) {
            { if (srcareap) { free(srcareap); srcareap=((void *)0); } if (dirarea) { free(dirarea); dirarea=((void *)0); } };
      return(errp);
  }
  if (n) p++;
  ...
  nn2=0; l2=0;
        if (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit)) {
            for (; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); ) { l1=l1*10+(*p-'0'); p++, nn1++; }
      if (nn1) while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace)) p++;
            for (; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); ) { l2=l2*10+(*p-'0'); p++, nn2++; }
      if (nn2) while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace)) p++;
        }
        else while (*p) {
            if (*p == '>') break;
            if (strncmp(p,"markmax=",8) == 0) {
                for (p+=8; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); ) { l1=l1*10+(*p-'0'); p++, nn1++; }
                if (nn1) nn1+=8;
                while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace)) p++;
                continue;
            }
            if (strncmp(p,"flenmin=",8) == 0) {
                for (p+=8; ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISdigit); ) { l2=l2*10+(*p-'0'); p++, nn2++; }
                if (nn2) nn2+=8;
                while (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace)) p++;
                continue;
            }
  ...
  if (*p++ != '>') {
      ...
      return(errp);
        ...
        if (nn1) smumaxlen=(unsigned short)l1;
        if (nn2) fldlenmin=(unsigned short)l2;

            for (n=0, fldp=p; *fldp; fldp++, n++) {
                xp=fldp;
                if (*xp++ != '<') continue;
                if (*xp++ != '/') continue;
                nn=0; while (((*__ctype_b_loc ())[(int) ((*xp))] & (unsigned short int) _ISspace)) { xp++; nn++; }
                for (tagmatch=0; ((*__ctype_b_loc ())[(int) ((*xp))] & (unsigned short int) _ISdigit); xp++, nn++)
                    tagmatch=tagmatch*10+(*xp-'0');
                while (((*__ctype_b_loc ())[(int) ((*xp))] & (unsigned short int) _ISspace)) { xp++; nn++; }
                if (*xp++ != '>') continue;
  ...
  dirp->tag = (unsigned short)tag;
  dirp->pos = (unsigned short)(p-batchp)-n;

                p+=endmatch;
                n-=erased;

  dirp->len = (unsigned short)n;
  *sp = 'b';
  vlenadd+=dirp->len;
  dirsleft++;
  nadd++;
 ...
 default:
    ...
    if ((long)(recp->recmf.m1.m1mfrl+(nadd-ndel)*sizeof(DIRSTRU)-vlendel+vlenadd) >
        (long)recp->recnbytes) {
 ...
 return(batchp);
    ...
    if (ndel) {
 mfdirp=recp->recmf.m1.m1dir;
 newp=recp->recmf.mx+recp->recmf.m1.m1base-(k=sizeof(DIRSTRU)*ndel); lastpos=0;
 sp=srcareap;
 for (dirp=dirarea, n=dirsleft; n--; dirp++, sp++)
  ...
  mfdirp->pos = lastpos;
  mfdirp->len = dirp->len;
  p=recp->recmf.mx+recp->recmf.m1.m1base+dirp->pos;
  memcpy(newp,p,dirp->len); newp+=dirp->len;
  lastpos+=dirp->len;
  mfdirp++;
     }
 recp->recmf.m1.m1base-=k;
 recp->recmf.m1.m1nvf-=ndel;
 recp->recmf.m1.m1mfrl-=(k+vlendel);
    }

    if (nadd) {
 nxtpos=recp->recmf.m1.m1mfrl-recp->recmf.m1.m1base;
 nxtp=recp->recmf.mx+recp->recmf.m1.m1mfrl+(k=sizeof(DIRSTRU)*nadd);
 for (p=(mfp=nxtp-1)-k, mfdirp=recp->recmf.m1.m1dir, n=recp->recmf.m1.m1nvf; n--; ) {
     for (loop=mfdirp->len; loop--; )
      *mfp-- = *p--;

     mfdirp++;
 }
 if (mfdirp != &recp->recmf.m1.m1dir[recp->recmf.m1.m1nvf])
     fatal("fldupdat/bug");
 sp=srcareap;
 ...
 recp->recmf.m1.m1base+=k;
 recp->recmf.m1.m1nvf+=nadd;
 recp->recmf.m1.m1mfrl+=(k+vlenadd);
    }
# 2080 "ciupd.c"
    { if (srcareap) { free(srcareap); srcareap=((void *)0); } if (dirarea) { free(dirarea); dirarea=((void *)0); } };
    ...
    if (sort) {
        ...
        srcareap=(char *)malloc((unsigned)(recp->recmf.m1.m1mfrl-recp->recmf.m1.m1base+1));

        if (srcareap == (char *)((void *)0)) fatal("fldupdat/ALLOC/sort");
        dirarea=((void *)0);
 dirp=recp->recmf.m1.m1dir;
 sp=srcareap;
 lastpos=0;
 for (n=recp->recmf.m1.m1nvf; n--; dirp++) {
     p=recp->recmf.mx+recp->recmf.m1.m1base+dirp->pos;
     memcpy(sp,p,dirp->len); sp+=dirp->len;
     dirp->pos = lastpos;
     lastpos+=dirp->len;
 }
 memcpy(recp->recmf.mx+recp->recmf.m1.m1base,srcareap,recp->recmf.m1.m1mfrl-recp->recmf.m1.m1base);
    { if (srcareap) { free(srcareap); srcareap=((void *)0); } if (dirarea) { free(dirarea); dirarea=((void *)0); } };
    }
