...

{
    ...
    gizmp=(GIZMSTRU *)malloc((unsigned)n);


    if (gizmp == (GIZMSTRU *)((void *)0))
...

{
    ...
    if (n) {
        ...
        p=(unsigned char *)malloc((unsigned)n);


        if (p == (unsigned char *)((void *)0))
...

{
    ...
    vgizmap=(VGIZPSTRU *)malloc((unsigned)nbytes);

    if (vgizmap == (VGIZPSTRU *)((void *)0))
        fatal("gizmread/ALLOC");
    memset(vgizmap,0x00,(unsigned)nbytes);
    *vgizmpp=vgizmap;
    for (igiz=0; igiz < 256; igiz++) {
        vgizmap->ghdrp[igiz] = ((void *)0);
    }
    vgizmap->grngs=0;
...
# 184 "cigiz.c"
    if ((cq=(unsigned char *)strchr(gizdbnp+1,c=',')) == ((void *)0)) {
        c='\0'; q=(unsigned char *)"";
    }
    else {
        *cq='\0'; q=cq+1;
    ...
    gmap=vgizmap;
# 200 "cigiz.c"
    for (mfn=1L; (recp=vrecp[(unsigned)record(irec,gizdbnp,mfn)])->recrc != (-1); mfn++) {


        if (recp->recrc != 0)
            continue;
        if (recp->recmf.m1.m1nvf == 0)
            continue;
        ...
        if ((n=nocc(irec,11)) > 0) {
           ...
           if (n != 1) {
                if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
                fatal("gizmread/GIZMTAG11/nocc");
           }



           xdir=fieldx(irec,11,1);

           if (recp->recmf.m1.m1dir[xdir].len > 3) {
                if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
                fatal("gizmread/GIZMTAG11/3");
           }


           strncpy((char *)gfld11,recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos),recp->recmf.m1.m1dir[xdir].len);
           gfld11[recp->recmf.m1.m1dir[xdir].len]='\0';
           if (strcmp(( char *)gfld11,"hex") != 0 && strcmp(( char *)gfld11,"asc") != 0) {
                if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
        ...
        if ((n=nocc(irec,21)) > 0) {
           ...
           strncpy((char *)gfld21,recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos),recp->recmf.m1.m1dir[xdir].len);
           gfld21[recp->recmf.m1.m1dir[xdir].len]='\0';
           if (strcmp(( char *)gfld21,"hex") != 0 && strcmp(( char *)gfld21,"asc") != 0) {
                if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
        ...
        xdir=fieldx(irec,1,1);


        switch (*gfld11) {
        case '\0':
            if ((fld1len=recp->recmf.m1.m1dir[xdir].len) < 1) {
                if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
                fatal("gizmread/GIZMTAG1/len");
            }
            fld1p=(unsigned char *)recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos);
            break;
        case 'h':
            if ((fld1len=recp->recmf.m1.m1dir[xdir].len) < 2 || fld1len & 0x01) {
                if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
                fatal("gizmread/GIZMTAG1/hexlen");
            }
            fld1p=(unsigned char *)recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos);
            for (p=fld1p, p2=p, n=fld1len; n; n-=2, p2+=2) {
                if (sscanf(( char *)p2,"%2x",&i) != 1) {
                    if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
            ...
            break;
        case 'a':
            if ((fld1len=recp->recmf.m1.m1dir[xdir].len) < 3 || (fld1len/3*3 != fld1len)) {
                if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
                fatal("gizmread/GIZMTAG1/asclen");
            }
            fld1p=(unsigned char *)recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos);
            for (p=fld1p, p2=p, n=fld1len; n; n-=3, p2+=3) {
                if (sscanf(( char *)p2,"%3d",&i) != 1) {
                    if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
        ...
        if (n > 0) {
            ...
            xdir=fieldx(irec,2,1);


            switch (*gfld21) {
            case '\0':
                if ((fld2len=recp->recmf.m1.m1dir[xdir].len) < 1) {
                    if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
                    fatal("gizmread/GIZMTAG2/len");
                }
                fld2p=(unsigned char *)recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos);
                break;
            case 'h':
                if ((fld2len=recp->recmf.m1.m1dir[xdir].len) < 2 || fld2len & 0x01) {
                    if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
                ...
                break;
            case 'a':
                if ((fld2len=recp->recmf.m1.m1dir[xdir].len) < 3 || (fld2len/3*3 != fld2len)) {
                    if (rectrace) fprintf(stderr,"mfn=%ld\n",mfn);
        ...
        for (gizmbp=((void *)0); gizmnp; gizmbp=gizmnp, gizmnp=gizmnp->nextp)
...
# 426 "cigiz.c"
    ...
    if (rectrace) {
        ...
        printf("gizmread - %s=%ld bytes\n",(*vgizmpp)->gdbnp,ubytes);
            ...
            while (*q) {
  if (gmap->grngs >= 16) fatal((char *)p);
                if (sscanf(( char *)q,"%ld/%ld",&xtag1,&xtag2) == 2) {
      if (xtag1 < 1 || xtag2 > (32767 * 2 + 1)) fatal((char *)p);
      if (xtag1 > xtag2) fatal((char *)p);
      while (*q != '/') q++; q++;
      while (((*__ctype_b_loc ())[(int) ((*q))] & (unsigned short int) _ISdigit)) q++;
      gmap->grng1[gmap->grngs]=(unsigned short)xtag1;
      gmap->grng2[gmap->grngs]=(unsigned short)xtag2;
      if (*q == ',') if (*(q+1)) q++;
      gmap->grngs++;
      continue;
  }
                if (sscanf(( char *)q,"%ld",&xtag1) == 1) {
      while (((*__ctype_b_loc ())[(int) ((*q))] & (unsigned short int) _ISdigit)) q++;
      if (xtag1 > (32767 * 2 + 1)) fatal((char *)p);
      gmap->grng1[gmap->grngs]=(unsigned short)xtag1;
      gmap->grng2[gmap->grngs]=(unsigned short)xtag1;
      if (*q == ',') if (*(q+1)) q++;
      gmap->grngs++;
      continue;
  }
...

{
 ...
 fatal("recgizmo/irec");
    ...
    if (recp->rectype != 12) {
        if (rectrace) printf("*** irec=%ld  RECtype=%d\n",irec,recp->rectype);
        fatal("recgizmo/check");
    }


    nbytes=(32767L +32767L);
    ...
    buffup=(char *)malloc((unsigned)nbytes);

    if (buffup == (unsigned char *)((void *)0))
        fatal("recgizmo/ALLOC");

    nmatchs=0;
    if (vgizmap->ghdrp == ((void *)0)) fatal("recgizmo/ghdrp");

    for (batchp=buffup, xdir=0, dirloop=recp->recmf.m1.m1nvf; dirloop--; xdir++) {
        tag=recp->recmf.m1.m1dir[xdir].tag; rngok=1;
        ...
        if (rngok) {
            sprintf((char *)batchp,"H%u 000000 ",tag); batchp+=strlen(( char *)batchp);
            fuplenp=batchp-1-6; fuplen=0;



            for (p=(unsigned char *)recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos), fldloop=recp->recmf.m1.m1dir[xdir].len; fldloop; ) {
            ...
            sprintf((char *)batchp,"H%u %u ",tag,recp->recmf.m1.m1dir[xdir].len); batchp+=strlen(( char *)batchp);
            memcpy(batchp,recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos),recp->recmf.m1.m1dir[xdir].len); batchp+=recp->recmf.m1.m1dir[xdir].len;
        }
    ...
    *batchp='\0';
    ...
    if (nmatchs) {
        ...
        recp->recmf.m1.m1mfrl=recp->recmf.m1.m1base=20; recp->recmf.m1.m1nvf=0;
...

{
    ...
    if (dectrace)
        printf("recdecod - 1. %s %p +%ld\n",dbxp->dbxname,dbxp->dbxvderp,coreleft());
# 1052 "cigiz.c"
for (wmfn=1; (recp=vrecp[(unsigned)record(wrec,decdbnp,wmfn)])->recrc != (-1); wmfn++) {


    if (recp->recrc != 0) continue;
# 1065 "cigiz.c"
    n=sizeof(VDECPSTRU);
    ...
    dmap=(VDECPSTRU *)malloc((unsigned)n);


    if (dmap == (VDECPSTRU *)((void *)0))
        fatal("recdecod/ALLOC/0");
    memset(dmap,0x00,(unsigned)n);

    if (dbxp->dbxvderp == ((void *)0)) dbxp->dbxvderp=dmap;
    else {
        for (lastdmap=dbxp->dbxvderp; lastdmap->nextp; )
    ...
    if ((wdir=fieldx(wrec,wtag=1,wocc=1)) < 0) {

        if (rectrace) fprintf(stderr,"*** %s/%ld/%d/%d \n",dbxp->dbxname,wmfn,wtag,wocc);
        fatal("recdecod/DECOTAG1");
    }
    n=recp->recmf.m1.m1dir[wdir].len;
    ...
    dmap->ddbnp=(unsigned char *)malloc((unsigned)n+1);


    if ((dmap->ddbnp == (unsigned char *)((void *)0)))
        fatal("recdecod/ALLOC/dmap");
    memcpy(dmap->ddbnp,recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[wdir].pos),n); dmap->ddbnp[n]='\0';

    if (dectrace)
        printf("recdecod - 2. %s=%p +%ld\n",dmap->ddbnp,dmap,coreleft());





    if ((wdir=fieldx(wrec,wtag=2,wocc=1)) < 0) dmap->drdlm = '\0';


    else if (recp->recmf.m1.m1dir[wdir].len == 0) dmap->drdlm = '\0';
         else dmap->drdlm = *(recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[wdir].pos));
    ...
    datp=(char *)malloc((unsigned)32767L);

    if (datp == (unsigned char *)((void *)0))
    ...
    wtag=3; dmap->drngs=nocc(wrec,wtag);


    if ((wocc=dmap->drngs) > 16) {
        if (rectrace) fprintf(stderr,"*** %s/%ld/%d/%d \n",dbxp->dbxname,wmfn,wtag,wocc);
        fatal("recdecod/DECOTAG3/nocc");
    }
    for (irng=0, wocc=1; wocc <= dmap->drngs; wocc++, irng++) {
        if (irng >= 16) fatal("recdecod/MAXDECR");




        wdir=fieldx(wrec,wtag=3,wocc);


        if (wdir < 0) fatal("recdecod/internal error");
        memcpy(datp,recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[wdir].pos),n=recp->recmf.m1.m1dir[wdir].len); datp[n]='\0';
        dmap->drng1[irng]=0;
        if (sscanf(( char *)datp,"%ld/%ld",&xtag1,&xtag2) == 2)
            if (xtag1 >= 1 && xtag2 <= (32767 * 2 + 1))
                if (xtag1 <= xtag2) {
                    dmap->drng1[irng]=(unsigned short)xtag1;
                    dmap->drng2[irng]=(unsigned short)xtag2;
                }
        if (sscanf(( char *)datp,"%ld",&xtag1) == 1)
            if (xtag1 <= (32767 * 2 + 1)) {
                dmap->drng1[irng]=(unsigned short)xtag1;
                dmap->drng2[irng]=(unsigned short)xtag1;
            }
    ...
    if ((wocc=dmap->dsflds=nocc(wrec,wtag=4)) > 16) {
    ...
    for (wocc=1; wocc <= dmap->dsflds; wocc++) {
        ...
        wdir=fieldx(wrec,wtag=4,wocc); wox=wocc-1;


        memcpy(dmap->dsfld,recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[wdir].pos),n=recp->recmf.m1.m1dir[wdir].len);
        dmap->dsfld[n]='\0';
        for (p=dmap->dsfld; *p; p++) *p=isisuctab[*p];
        if ((p=(unsigned char *)strchr((char *)dmap->dsfld,'^')) != ((void *)0))
            if (*++p) {
                dmap->dsfldx[wox] = *p;
                dmap->dsfldv[wox] = atol(( char *)(p+1));
            }
    }
    ...
    if (dectrace)
        printf("recdecod - 3. %s %p +%ld\n",dbxp->dbxname,dbxp->dbxvderp,coreleft());

    for (dmap=dbxp->dbxvderp; dmap; dmap=dmap->nextp)
...

{
    ...
    if (dectrace)
        printf("recdecex - irec=%ld=%s/%ld +%ld\n",
            irec,recp->recdbxp->dbxname,recp->recmf.m1.m1mfn,coreleft());
    ...
    for (xdir=0, dirloop=recp->recmf.m1.m1nvf; dirloop--; xdir++) {
        tag=recp->recmf.m1.m1dir[xdir].tag;
# 1322 "cigiz.c"
        for (imap=0, dmap=recp->recdbxp->dbxvderp; dmap; dmap=dmap->nextp, imap++) {
            rngok=1;
        ...
        if (rngok) {
            if (dectrace) {
                printf("recdecex - irec=%ld=%s/%ld %s,%02x=%c,",
                    irec,recp->recdbxp->dbxname,recp->recmf.m1.m1mfn,dmap->ddbnp,dmap->drdlm,dmap->drdlm);
                printf("%d,",tag);
                for (i=0; i < dmap->dsflds; i++) {
                    printf("^%c",dmap->dsfld[i]);
                    if (dmap->dsfldx[i])
                        printf("+%c=%ld",dmap->dsfldx[i],dmap->dsfldv[i]);
                }
                printf("\n");
            }
            ...
            fuplenp=batchp-1-6; fuplen=0;
            sfldn=0; sfldc='\0';
            for (p=(unsigned char *)recp->recmf.mx+(recp->recmf.m1.m1base+recp->recmf.m1.m1dir[xdir].pos), fldloop=recp->recmf.m1.m1dir[xdir].len; fldloop; sfldn++) {
                rchar = ~(dmap->drdlm);



                for (sfldlen=1, q=p+1, n=fldloop-1; n--; q++)
                    if (*q == '^' || (rchar = *q) == dmap->drdlm) break;
                    else sfldlen++;
                if (sfldlen > 1)
                    if (*p == '^') {
                        sfldc = *(p+1);
                ...
                for (valmfn=0L, q=p, left=n=sfldlen; n--; q++, left--)
                    if (((*__ctype_b_loc ())[(int) ((*q))] & (unsigned short int) _ISdigit)) valmfn=valmfn*10+((int)*q-(int)'0');
                    else break;
                leftp=q;

                if (rchar == dmap->drdlm) sfldlen++;
                ...
                if (valmfn > 0L) {
                    ...
                    if (!decridx) {
                        ...
                        if (decridx > 0) recallok(decridx,32767L);
                        else valmfn=0L;
                    }
                ...
                if (valmfn > 0L) {
                    ...
                    R2recp=vrecp[record(((decridx)?decridx:(decridx=nrecs)),dmap->ddbnp,valmfn)];

                    if (R2recp->recmf.m1.m1nvf == 0) R2recp->recrc=1;
                    if (R2recp->recrc == 0) {
                        nmatchs++; match=1;
                        ...
                        if (dectrace)
                            printf("recdecex - %s/%ld/%d[%d] = %s/%ld",
                                recp->recdbxp->dbxname,recp->recmf.m1.m1mfn,recp->recmf.m1.m1dir[xdir].tag,xdir,
                                R2recp->recdbxp->dbxname,R2recp->recmf.m1.m1mfn);
                        decxdir=0;
                        for (; decxdir < R2recp->recmf.m1.m1nvf; decxdir++) {
                            if (dectrace) printf("/%d",R2recp->recmf.m1.m1dir[decxdir].tag);
                            q=(unsigned char *)vrecp[decridx]->recmf.mx+(R2recp->recmf.m1.m1base+R2recp->recmf.m1.m1dir[decxdir].pos);
                            memcpy(batchp,q,n=R2recp->recmf.m1.m1dir[decxdir].len);
                            batchp+=n; fuplen+=n;
                            if (left) {
    ...
    *batchp='\0';
    ...
    if (decridx) {
        free(vrecp[decridx]); vrecp[decridx]=((void *)0); nrecs--;
    }
 ...
 if (p) {
    ...
    if (dectrace)
       printf("recdecex - irec=%ld/%ld matchs=%ld +%ld\n",
           irec,recp->recmf.m1.m1mfn,nmatchs,coreleft());
