...
Dbx_read(DbxObject *self, PyObject *args)
{
  ...
  /* new record (RECSTRU) should know (DBXSTRU) where it belongs */  
...
newRecordObject(PyObject *arg)
{
    ...
    self->recp=(RECSTRU *)ALLOC((ALLOPARM)(RECHSIZE+rec_maxmfrl));
    if (self->recp == (RECSTRU *)ALLONULL) {
		PyErr_SetString(ErrorObject, "Failed to allocate recp.");
		Py_DECREF(self);
        return NULL;
    }
    memset(self->recp, 0x00, (size_t)(RECHSIZE+rec_maxmfrl));
