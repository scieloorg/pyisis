...
def test_copyDb():
    for ext in ('.mst','.xrf'):
        fname = join("..","sample",'copycds%s' % ext)
        if exists(fname):
            remove(fname)
        fname = join("..","sample",'copycdsFFI%s' % ext)
        if exists(fname):
            remove(fname)
    mf_from = MasterFile(join("..","sample","cds.mst"))
    last = mf_from.nxtmfn - 1

    fname = join("..","sample",'copycds')
    f = open(fname + ".ini", "w")
    f.write("[Engine]\nINPUT_ENCODING = cp850\nOUTPUT_ENCODING = utf-8\n[IsisDB]\nLEADER_XL=True")
    f.close()
    mf_to = MasterFile(join("..","sample","copycds.mst"))
    ...
    fname = join("..","sample",'copycdsFFI')
    f = open(fname + ".ini", "w")
    f.write("[Engine]\nINPUT_ENCODING = cp850\nOUTPUT_ENCODING = utf-8\n[IsisDB]\nLEADER_XL=True")
    f.write("\nDIR_MASK = iii\nLEADER_MASK_XL = iiHHiiHH")
    f.close()
    mf_to = MasterFile(join("..","sample","copycdsFFI.mst"))
    for mfn in range(1, last):
        rec = mf_from[mfn]
        rec.save(mf_to)
    del mf_to
